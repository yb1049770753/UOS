...
def linux_screenshot(bbox=None):
    ...
    if bbox:
        ...
        if width > 0 and height > 0:
            return Image.new('RGB', (width, height), (50, 50, 50))
    ...
    def handle_screen(self, conn):
        ...
        while self.is_running:
            try:
                ...
                if len(data) < 100:
                    print(f"[画面传输] 图片数据太小: {len(data)} bytes")
                    time.sleep(0.1)
                    continue
                ...
                frame_count += 1
                if frame_count % 25 == 0:  # 每秒打印一次
                    print(f"[画面传输] 已发送 {frame_count} 帧, 当前 {len(data)} bytes")
                time.sleep(0.04)  # 约 25 帧/秒
            except Exception as e:
                print(f"[画面传输] 错误: {e}")
                break
        print(f"[画面传输] 客户端断开, 共发送 {frame_count} 帧")
        conn.close()
